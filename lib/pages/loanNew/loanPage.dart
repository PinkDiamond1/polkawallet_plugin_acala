...
class _LoanPageState extends State<LoanPage> {
  ...
  Future<Map?> _getTxParams(
    ...
    final balancePair = AssetsUtils.getBalancePairFromTokenNameId(
        widget.plugin, [loan.token!.symbol, acala_stable_coin]);
    final dic = I18n.of(context)!.getDic(i18n_full_dic_acala, 'acala');
    ...
    if (debitShares != BigInt.zero) {
      ...
      detail[dic![dicValue]!] = Text(
        '${Fmt.priceFloorBigInt(loan.type.debitShareToDebit(debitSubtract).abs(), balancePair[0]!.decimals!, lengthMax: 4)} ${PluginFmt.tokenView(acala_stable_coin)}',
        style: Theme.of(context)
            .textTheme
            .headline1
            ?.copyWith(color: Colors.white),
                          ...
                          if (loan != null) {
                            final balancePair =
                                AssetsUtils.getBalancePairFromTokenNameId(
                                    widget.plugin,
                                    [loan.token!.symbol, acala_stable_coin]);
