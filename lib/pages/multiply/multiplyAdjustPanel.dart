...
import 'package:polkawallet_plugin_acala/pages/multiply/slider/multiplySliderOverlayShape.dart';
import 'package:polkawallet_plugin_acala/pages/multiply/slider/multiplySliderThumbShape.dart';
import 'package:polkawallet_plugin_acala/pages/multiply/slider/multiplySliderTickMarkShape.dart';
import 'package:polkawallet_plugin_acala/pages/multiply/slider/multiplySliderTrackShape.dart';
...
class _MultiplyAdjustPanelState extends State<MultiplyAdjustPanel> {
    ...
    return {
      'detail': {
        ...
        dic['loan.multiply.outstandingDebt']: Text(
          '${Fmt.priceCeilBigInt(debitNew - raisingDebit, balancePair[1].decimals!)} $acala_stable_coin_view',
          style: Theme.of(context)
              .textTheme
              .headline1
              ?.copyWith(color: PluginColorsDark.headline1),
  ...
  Map _getSellingParams(LoanType loanType, List<TokenBalanceData> balancePair,
    ...
    final detail = {
      ...
      dic['loan.multiply.outstandingDebt']: Text(
        '≈ ${Fmt.priceCeilBigInt(debitNew, balancePair[1].decimals!)} $acala_stable_coin_view',
        style: Theme.of(context)
            .textTheme
            .headline1
            ?.copyWith(color: PluginColorsDark.headline1),
  ...
  Future<void> _onSubmit(LoanType loanType, BigInt collateralChange,
    ...
    final balancePair = AssetsUtils.getBalancePairFromTokenNameId(
        widget.plugin, [widget.loanType.token?.tokenNameId, acala_stable_coin]);
  ...
  Future<void> _updateDexBuyingPrice() async {
    final res = await widget.plugin.api!.swap.queryTokenSwapAmount(
        null,
        '1',
        [acala_stable_coin, widget.loanType.token!.tokenNameId!].map((e) {
  ...
  Future<void> _updateDexSellingPrice() async {
    final res = await widget.plugin.api!.swap.queryTokenSwapAmount(
        '1',
        null,
        [widget.loanType.token!.tokenNameId!, acala_stable_coin].map((e) {
          final token = AssetsUtils.getBalanceFromTokenNameId(widget.plugin, e);
          return {...token.currencyId!, 'decimals': token.decimals};
        }).toList(),
        '0.05');
          ...
          children: <Widget>[
            ...
            ErrorMessage(
                debitNew < loanType.minimumDebitValue
                    ? '${assetDic!['min']} ${Fmt.bigIntToDouble(loanType.minimumDebitValue, balancePair[1].decimals!).toStringAsFixed(2)} ${PluginFmt.tokenView(acala_stable_coin_view)}'
                    : null,
                margin: EdgeInsets.symmetric(vertical: 2),
                isRight: true),
            PluginTextTag(
                            ...
                            child: Column(
                              children: [
                                ...
                                MultiplyInfoItemRow(
                                  ...
                                  "${Fmt.priceFloorBigInt(debitNew, balancePair[1].decimals!, lengthMax: 4)} ${PluginFmt.tokenView(acala_stable_coin_view)}",
