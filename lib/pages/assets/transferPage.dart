...
class _TransferPageState extends State<TransferPage> {
  ...
  void _onSelectChain(Map<String, Widget> crossChainIcons) {
    ...
    final List tokenXcmConfig =
        (tokensConfig['xcm'] ?? {})[_token!.tokenNameId] ?? [];
    ...
    if (_accountToError == null &&
      ...
      if (chainTo != widget.plugin.basic.name) {
        ...
        return TxConfirmParams(
          ...
          module: 'xTokens',
          call:
              isFromStateMint == true ? 'transferMulticurrencies' : 'transfer',
          txDisplay: {
  ...
  String _getWarnInfo(String token) {
    return I18n.of(context)!.locale.toString().contains('zh')
        ? '交易所当前不支持 Acala 网络跨链转账充提 $token，请先使用跨链转账将 $token 转回 $_chainTo，再从 $_chainTo 网络转账至交易所地址。'
        : 'Exchanges do not currently support direct transfers of $token to/from Acala. In order to successfully send $token to an exchange address, it is required that you first complete an Cross-Chain-Transfer of the token(s) from Acala to $_chainTo.';
  }
  ...
  Widget build(BuildContext context) {
    return Observer(
      builder: (_) {
        ...
        final isToMoonBeam = chainTo == para_chain_name_moon;
        ...
        final chainToId = isCrossChain
            ? tokensConfig['xcmChains'][chainTo]['id']
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Visibility(
                      visible: isCrossChain && isFromStateMint,
                      child: _CrossChainTransferWarning(
